...
public class CalculatorActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        history = new TextView(this); history.setTextSize(13); history.setTextColor(Color.DKGRAY); history.setPadding(dp(12),dp(10),dp(12),dp(10)); history.setBackground(round(Color.WHITE,10)); history.setText(getSharedPreferences("dig_calculator",MODE_PRIVATE).getString("calc_history","Historico vazio")); root.addView(history);
    ...
    private void confirmClearHistory(){new android.app.AlertDialog.Builder(this).setTitle("Apagar historico?").setMessage("Todos os calculos salvos nesta tela serao removidos.").setNegativeButton("Cancelar",null).setPositiveButton("Apagar",(d,w)->{getSharedPreferences("dig_calculator",MODE_PRIVATE).edit().remove("calc_history").apply();history.setText("Historico vazio");}).show();}
    private void migrateAndCleanOldGameData(){android.content.SharedPreferences old=getSharedPreferences("dig_games",MODE_PRIVATE);android.content.SharedPreferences calc=getSharedPreferences("dig_calculator",MODE_PRIVATE);if(!calc.contains("calc_history")&&old.contains("calc_history"))calc.edit().putString("calc_history",old.getString("calc_history","")).apply();old.edit().clear().apply();}
